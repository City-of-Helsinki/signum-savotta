    ...
    def parse_data(self, data_bytes):
        ...
        try:
            while len(binary_string) > 0 and len(binary_string) != prev_length:
                ...
                match oid:
                    case Oid.PRIMARY_ITEM_IDENTIFIER: self.primary_item_identifier = value
                    case Oid.CONTENT_PARAMETER: self.content_parameter = value
                    case Oid.OWNER_LIBRARY_ISIL: self.owner_library_isil = value
                    case Oid.SET_INFORMATION: self.set_information = value
                    case Oid.TYPE_OF_USAGE: self.type_of_usage = value
                    case Oid.SHELF_LOCATION: self.shelf_location = value
                    case Oid.ONIX_MEDIA_FORMAT: self.onix_media_format = value
                    case Oid.MARC_MEDIA_FORMAT: self.marc_media_format = value
                    case Oid.SUPPLIER_IDENTIFIER: self.supplier_identifier = value
                    case Oid.ORDER_NUMBER: self.order_number = value
                    case Oid.ILL_BORROWING_INSTITUTION_ISIL: self.ill_borrowing_institution_isil = value
                    case Oid.ILL_BORROWING_TRANSACTION_NUMBER: self.ill_borrowing_transaction_number = value
                    case Oid.GS1_PRODUCT_IDENTIFIER: self.gs1_product_identifier = value
                    case Oid.ALTERNATIVE_UNIQUE_ITEM_IDENTIFIER: self.alternative_unique_item_identifier = value
                    case Oid.LOCAL_DATA_A: self.local_data_a = value
                    case Oid.LOCAL_DATA_B: self.local_data_b = value
                    case Oid.TITLE: self.title = value
                    case Oid.PRODUCT_IDENTIFIER_LOCAL: self.product_identifier_local = value
                    case Oid.MEDIA_FORMAT_OTHER: self.media_format_other = value
                    case Oid.SUPPLY_CHAIN_STAGE: self.supply_chain_stage = value
                    case Oid.SUPPLIER_INVOICE_NUMBER: self.supplier_invoice_number = value
                    case Oid.ALTERNATIVE_ITEM_IDENTIFIER: self.alternative_item_identifier = value
                    case Oid.ALTERNATIVE_OWNER_LIBRARY_IDENTIFIER: self.alternative_owner_library_identifier = value
                    case Oid.SUBSIDIARY_OF_AN_OWNER_LIBRARY: self.subsidiary_of_an_owner_library = value
