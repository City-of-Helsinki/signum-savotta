...
class BrotherPt:
    ...
    def __initialize(self):
        ...
        # Local modification (vendored fork): kernel-driver detach is a
        # Linux/macOS concept; libusb on Windows raises NotImplementedError
        # for both is_kernel_driver_active() and detach_kernel_driver().
        # Suppress so this code runs cross-platform.
        try:
    ...
    def __del__(self):
        # Local modification (vendored fork): guard against partial init.
        # If __init__ raises before self._dev is assigned (e.g. no device
        # found), the original code crashed here with AttributeError during
        # garbage collection. See vendor/brother_pt/__init__.py for vendoring
        # provenance.
    ...
    def update_status(self):
        # Local modification (vendored fork): invalidate() is 100 zero bytes,
        # documented as a printer reset. On PT-P710BT, after the printer has
        # entered raster mode (from a prior print), these 100 zeros appear to
        # be interpreted as 100 blank raster lines and ejected as a fixed-width
        # ~14 mm blank label preceding every actual print. \x1B\x40 (initialize)
        # alone is sufficient to reset the command interpreter for a status
        # query.
    ...
    def print_data(self, data:bytes, margin_px:int):
        # Local modification (vendored fork): set_advanced_mode() sends
        # \x1B\x69\x4B\x08 which the upstream comment labels "no chain
        # printing", but on PT-P710BT firmware that bit position appears to
        # be "Special tape" mode and causes a spurious second cut/feed
        # (observed: a blank label is ejected after the printed one).
        # Omitting the call lets the printer use its default mode bits.
        ...
        # Local modification (vendored fork): honour an optional `_mode`
        # instance attribute so callers can disable auto-cut for chain
        # printing. Defaults to Mode.AUTO_CUT (upstream behaviour).
        self.__write(set_mode(getattr(self, "_mode", Mode.AUTO_CUT)))
        ...
        for cmd in gen_raster_commands(data):
            self.__write(cmd)
        # Local modification (vendored fork): honour an optional `_final_cmd`
        # instance attribute so callers can swap \x1A (print and feed; cuts
        # on PT-P710BT) for \x0C (print without feeding; no cut). Used by
        # the "no cuts ever" mode for batched chain printing.
        final_cmd = getattr(self, "_final_cmd", print_with_feeding)
        self.__write(final_cmd())
        while True:
            res = self.__read()
            if len(res) > 0:
